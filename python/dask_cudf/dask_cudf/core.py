...
if QUERY_PLANNING_ON:
    # Redirect to dask_cudf.expr
    from dask_cudf.expr._collection import (  # noqa: F841
        DataFrame,
        Index,
        Series,
    )
    ...
    class DataFrame(_Frame, dd.core.DataFrame):
        ...
        @_dask_cudf_nvtx_annotate
        def apply_rows(
            self, func, incols, outcols, kwargs=None, cache_key=None
        ):
            import uuid
    ...
    def reduction(
        ...
        **kwargs,
        ...
        if len(npartitions) > 1:
            raise ValueError(
                "All arguments must have same number of partitions"
            )
        ...
        if len(args) == 1 and isinstance(args[0], _Frame) and not chunk_kwargs:
            ...
            dsk = {
                (a, 0, i): (
                    ...
                    chunk,
                    [
                        (x._name, i) if isinstance(x, _Frame) else x
                        for x in args
                    ],
                    chunk_kwargs,
                )
        ...
        depth = 0
        while k > split_every:
            for part_i, inds in enumerate(
                partition_all(split_every, range(k))
            ):
...
###
### Functions used in both legacy and new API
###
