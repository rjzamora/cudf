...
def _get_unique_fractions(
    ...
    *,
    ...
    for c in set(column_names).intersection(column_stats):
        if row_count and column_stats[c].source_info.unique_count_estimate.value is not None:
            # Use implied unique-count estimate (if available)
            unique_fractions[c] = max(
                min(1.0, column_stats[c].source_info.unique_count_estimate.value / row_count.value),
                0.00001,
            )
