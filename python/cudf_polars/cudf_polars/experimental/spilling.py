...
def unspill_and_evaluate(
    ...
    *args: DataFrame | Spillable,
    ...
    # io_task = not args
    # n_spilled = len(
    #     [isinstance(obj, Spillable) and obj.mem_type() == MemoryType.HOST for obj in args]
    # ) if MemoryType is not None else 0
    ...
    #             available = ctx.br.device_memory_available
    #             if available < headroom:
    #                 spilled = ctx.br.spill_manager.spill_to_make_headroom(headroom=headroom)
