...
async def drain_metadata_passthrough_data(
    ...
    async with shutdown_on_error(ctx, ch_in.metadata, ch_in.data, ch_out):
        # Drain metadata (we don't need it in the final output).
        # In the future, we can optionally print this information
        # for debugging purposes.
        _ = await ch_in.recv_metadata(ctx)
...
def generate_network(
    ...
    # Add a node to drain metadata and pass through data
    ch_data_only = Channel()
    nodes.append(
        drain_metadata_passthrough_data(
            ctx,
            ch_in=channels[ir].pop(),
            ch_out=ch_data_only,
        )
    )
