...
class ParquetScanTask(ScanTask):
    ...
    def _ordered_row_group_indices(
        ...
        for groups, info in zip(bounds.row_groups, cached_info, strict=True):
            ...
            for group in groups:
                ...
                if chunk is None:
                    return None
                null_count = chunk.meta_data.statistics.null_count
    ...
    @staticmethod
    def _read_column_chunk_bounds(
        cached_info: Sequence[CachedParquetInfo],
        column: str,
        stream: Stream,
    ) -> tuple[plc.Column, plc.Column] | None:
        """Read parquet min/max bounds for one physical column."""
        try:
    ...
    @staticmethod
    def _task_boundary_column(
        min_column: plc.Column,
        max_column: plc.Column,
        row_group_indices: list[int],
        order: plc.types.Order,
        null_order: plc.types.NullOrder,
        stream: Stream,
    ) -> plc.Column | None:
        """Return first/last task boundaries if row-group bounds are ordered."""
    ...
    def get_ordered_boundaries(
        ...
        bounds = self._get_task_bounds(cached_info)
        row_group_indices = self._ordered_row_group_indices(cached_info, column, bounds)
        if row_group_indices is None:
            return None
        min_max = self._read_column_chunk_bounds(cached_info, column, stream)
        if min_max is None:
            return None
        return self._task_boundary_column(
            *min_max, row_group_indices, order, null_order, stream
        )
